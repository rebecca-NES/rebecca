    ...
    _proto._init = function() {
        ...
        if(_myJid == _ownJid) {
            ...
            _self._dialogInnerElement.find('.scroll_content').on('ps-y-reach-end', function(){
                ...
                for(let i=0;
                    (
                        followeeUser.length < Conf.getVal('NUMBER_OF_ITEMS_BY_PER_REQUEST') &&
                        (i + doneListCount) < _followeeList.getCount() &&
                        (doneListCount % (Conf.getVal('NUMBER_OF_ITEMS_BY_PER_REQUEST') - 1) == 0)
                    );
                    i++){
                    let _id = i + doneListCount;
                    if(_self._searchword.length > 0){
                        if( _followeeList.get(_id).getUserName().indexOf(_self._searchword) >= 0 ||
                            _followeeList.get(_id).getGroup().indexOf(_self._searchword) >= 0 ||
                            _followeeList.get(_id).getLoginAccount().indexOf(_self._searchword) >= 0){
                            followeeUser.push(_followeeList.get(_id));
                        }
                    }else{
                        followeeUser.push(_followeeList.get(_id));
                    }
    ...
    _proto.filterColumnName = function(){
        ...
        for(var i = 0 ; i < _followeeAllList ; i++){
            ...
            if(followee.getUserName().indexOf(_inputKeyword) >= 0 ||
                ...
                followee.getLoginAccount().indexOf(_inputKeyword) >= 0){
