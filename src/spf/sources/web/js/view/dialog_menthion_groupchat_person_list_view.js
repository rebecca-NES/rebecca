...
function DialogMenthionGroupChatPersonList(roomId, baseMessage, doneFunction) {
    var _self = this;
    _self._roomId = roomId;
    _self._baseMessage = baseMessage;
    _self.doneFunction = doneFunction;
    var _title = Resource.getMessage('menthion_groupchat_person_list_dialog_title');
    _self._autoCompleteCssCls = 'autocomplete-for-chatroom';
    DialogSelectChatRoomMemberView.call(this, _title, roomId);
};(function() {
    DialogMenthionGroupChatPersonList.prototype = $.extend({}, DialogSelectChatRoomMemberView.prototype);
    var _super = DialogSelectChatRoomMemberView.prototype;
    var _proto = DialogMenthionGroupChatPersonList.prototype;
    ...
    _proto._init = function() {
       ...
       _self._dialogInnerElement.find('input[name=allcheck]').on('change', function() {
           ...
           let jids = _self._dialogInnerElement.find('input[name=mentionmember]');
           if(this.checked){
               for( var i = 0; i < jids.length; i++){
                   if(0 === _self.memberList._array.filter(m => m === jids[i].attr("uid")).length){
                       _self.memberList.add(jids[i].attr("uid"));
                   }
               }
           }else{
               for( var i = 0; i < jids.length; i++){
                   _self.memberList.remove(_self.memberList._array.indexOf(jids[i].attr("uid")));
               }
           }
    ...
    _proto.getHtml = function(){
        const ret = '<div id="mentionmember_modal" class="card modal_card">\
          <div class="card_title">\
            <p>'+Resource.getMessage('menthion_groupchat_person_list_dialog_title')+'</p>\
          </div>\
