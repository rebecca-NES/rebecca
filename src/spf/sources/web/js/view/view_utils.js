    ...
    ViewUtils.sendPushNotification = function(_messageObject) {
        ...
        switch(_messageObject.getType()) {
            ...
            case Message.TYPE_QUESTIONNAIRE:
                ...
                break;
            case Message.TYPE_MURMUR:
                const flist = LoginUser.getInstance().getFolloweeList();
                let isFollowee = false;
                const myAcount = LoginUser.getInstance().getLoginAccount();
                const myJid = LoginUser.getInstance().getJid();
                for(let i=0;i< flist.getCount();i++){
                    if(
                       (flist.get(i).getJid() == _messageObject.getTo()) ||
                       (_messageObject.getTo() == myJid && _messageObject.getFrom() != myJid) ||
                       (_messageObject.getMessage().indexOf("@" + myAcount) >= 0) ||
                       (_messageObject.getMessage().indexOf("@all") >= 0)
                    ){
                ...
                _title += Resource.getMessage('Murmur') + ' - ' + _messageObject.getProfileByJid(_messageObject.getTo()).getNickName();
                break;
            default:
                return;
        }
