/**
 * ダイアログ関連
 * @class つぶやきメッセージ投稿ダイアログ
 * @constructor
 * @extends DialogSendMurmurMessageView
 * @param {String} message メッセージオブジェクト
 * @param {String} column カラムオブジェクト
 */
                ...
                CodiMdController.getInstance().getNoteList(
                    ...
                    if(res.result && Array.isArray(res.data)){
                        ...
                        for(let i=0;i<res.data.length;i++){
                            ...
                            const noteTitle = Utils.convertEscapedHtml(decodeURIComponent(res.data[i].note_title));
                            const noteRoom = res.data[i].room_name ?
                                             Utils.convertEscapedHtmldecodeURIComponent(res.data[i].room_name) :
                            ...
                            const a = $("<a>")
                                ...
                                .html('<div class="name note-list">'+noteTitle +'</div>'
                                    ...
                                    _self.isNoteLiistOpen = false;
                                    const selectedTitle = Utils.convertUnEscapedHtml($(event.currentTarget).attr("title"));
                                    const selectedValue = Utils.convertUnEscapedHtml($(event.currentTarget).attr("value"));
