...
function DialogChatListView() {
    ...
    _proto._init = function() {
        ...
        _self._dialogInnerElement.find('.scroll_content').on('ps-y-reach-end', function(){
            ...
            for(let i=0;
                (
                    personList.length < Conf.getVal('NUMBER_OF_ITEMS_BY_PER_REQUEST') &&
                    (i + doneListCount) < _contactList.getCount() &&
                    (doneListCount % (Conf.getVal('NUMBER_OF_ITEMS_BY_PER_REQUEST') - 1) == 0)
                )
                ;i++){
                let _id = i + doneListCount;
                if(_self._searchword.length > 0){
                    if(_contactList.get(_id).getUserName().indexOf(_self._searchword) >= 0 ||
                       _contactList.get(_id).getGroup().indexOf(_self._searchword) >= 0 ||
                       _contactList.get(_id).getLoginAccount().indexOf(_self._searchword) >= 0){
                        personList.push(_contactList.get(_id));
                    }
                }else{
                    personList.push(_contactList.get(_id));
                }
