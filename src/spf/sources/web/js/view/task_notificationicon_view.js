...
function TaskNotificationIconView(message) {
    this._demandTask = 0;
    this._normalTask = 0;
    NotificationIconView.call(this, message);
};(function() {
    TaskNotificationIconView.prototype = $.extend({}, NotificationIconView.prototype, ViewCore.prototype);
    var _super = NotificationIconView.prototype;
    var _proto = TaskNotificationIconView.prototype;
    ...
    _proto._setType = function() {
        var _self = this;
        _self._type = ColumnInformation.TYPE_COLUMN_TASK;
    };
    ...
    _proto.setDemandData = function(itemList) {
        var _self = this;
        var _selfElm = _self._htmlElement;
        var _targetElem = _selfElm.find(".alert_mytask");
        _self._demandTask = itemList.getByKey(ColumnInformation.TYPE_COLUMN_TASK.toString());
        if(_self.getTaskCount().toString() == "0"){
    ...
    _proto.updateNotificationTooltipMessage = function(message) {
        ...
        var _onGetContentCallback = function(content){
            ...
            _selfElm.append(_htmlString);
            _self._addEventClick(_selfElm.find(".alert_mytask"));
        };
    ...
    _proto._notificationTooltipMessageHtml = function(message, callback) {
        ...
        _htmlString += '<a  class="alert_mytask text_btn">';
        _htmlString += '<span class="ico ico_system">';
        _htmlString += '<i class="menu-column fa fa-check" title="マイタスク"></i>';
        _htmlString += '</span>';
        _htmlString += '<span class="name">マイタスク</span><span class="number">' + _self.makeNumber() + '</span>';
        _htmlString += '</a></li>';
    ...
    _proto._onListItemClicked = function() {
        ...
        if((_self.getTaskCount().toString() == "0") || (NotificationSettingManager.getInstance().isSetting(ColumnInformation.TYPE_COLUMN_TASK, ""))){
