...
function DialogFollowerListView(_jid) {
    ...
    _proto._init = function() {
        ...
        if(_myJid == _ownJid) {
            ...
            _self._dialogInnerElement.find('.scroll_content').on('ps-y-reach-end', function(){
                var followerUser = [];
                const doneListCount = _self._dialogInnerElement.find('ul.modal_list.select_list li').length;
                for(let i=0;
                    (
                        followerUser.length < Conf.getVal('NUMBER_OF_ITEMS_BY_PER_REQUEST') &&
                        (i + doneListCount) < _followerList.getCount() &&
                        (doneListCount % (Conf.getVal('NUMBER_OF_ITEMS_BY_PER_REQUEST') - 1) == 0)
                    );
                    i++){
                    let _id = i + doneListCount;
                    if(_self._searchword.length > 0){
                        if(_followerList.get(_id).getUserName().indexOf(_self._searchword) >= 0 ||
                           _followerList.get(_id).getGroup().indexOf(_self._searchword) >= 0 ||
                           _followerList.get(_id).getLoginAccount().indexOf(_self._searchword) >= 0){
                            followerUser.push(_followerList.get(_id));
                        }
                    }else{
                        followerUser.push(_followerList.get(_id));
                    }
    ...
    _proto._createColumnNameElement = function(columnNameList, followerAlllist, isNotZeroResult) {
        ...
        for (var i=0; i<columnNameList.length; i++) {
            if(typeof columnNameList[i] == 'object' &&
               columnNameList[i].getUserName() != null &&
    ...
    _proto.filterColumnName = function(){
        ...
        for(var i = 0 ; i < _followerAllList ; i++){
            ...
            if(follower.getUserName().indexOf(_inputKeyword) >= 0 ||
               ...
               follower.getLoginAccount().indexOf(_inputKeyword) >= 0){
                followerUser.push(follower);
            }
        }
        _self._searchword = _inputKeyword;
        _self._createColumnNameElement(followerUser, _followerAllList);
