...
public class LessThanCondition extends ItemCondition {
    public static final String ELEMENT_NAME = "lessthan";
    ...
    public String toSqlWhereSectionString() {
        ...
        if (mName == null || mType == null || mValue == null) {
            Log.error("LessThanCondition#toSqlWhereSectionString::data is invalid");
            return ret;
        }
        switch (mType) {
        case NUMBER:
        ...
        case STRING:
            ...
            break;
        default:
            Log.error("LessThanCondition#toSqlWhereSectionString::type is invalid");
            break;
        }
        return ret;
    }
    ...
    public boolean setDataFromElement(Element element) {
        if (element == null) {
            Log.error("LessThanCondition#setDataFromElement::element is null");
            return false;
        }
        String tagName = element.getName();
        if (tagName == null) {
            Log.error("LessThanCondition#setDataFromElement::tag name is null");
            return false;
        }
        if (!(tagName.toLowerCase().equals(ELEMENT_NAME))) {
            Log.error("LessThanCondition#setDataFromElement::tag name is different");
            return false;
        }
        List<?> childElems = element.elements();
        if (childElems != null && childElems.size() > 0) {
            Log.error("LessThanCondition#setDataFromElement::this elem has children - "
        ...
        if (conditionName == null || conditionName.equals("")) {
            Log.error("LessThanCondition#setDataFromElement::condition name is invalid");
            return false;
        }
        String conditionType = element.attributeValue(CONDITION_TYPE_ATTR);
        if (conditionType == null) {
            Log.error("LessThanCondition#setDataFromElement::condition type is null");
            return false;
        }
        String conditionValue = element.getStringValue();
        if (conditionValue == null) {
            Log.error("LessThanCondition#setDataFromElement::condition value is null");
            return false;
        }
        if (conditionType.equals(CONDITION_TYPE_ATTR_VALUE_NUMBER)) {
            ...
            Log.error("LessThanCondition#setDataFromElement::condition type is unknown");
            return false;
        }
