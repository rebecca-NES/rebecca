...
public class ItemCondition extends UserFilterCondition {
    ...
    public String toSqlWhereSectionString() {
        ...
        if(mName.equals(UserAccountStroreDbHelper.COLUMN_LOGIN_ACCOUNT)) {
            mName = UserProfileDbHelper.COLUMN_JID_NAME;
            mType = ValueType.STRING;
            UserAccountInfo accountInfo = UserAccountStroreDbHelper.getUserAccountInfo(mValue);
            if(accountInfo == null) {
                Log.info("ItemCondition#toSqlWhereSectionString::accountInfo is null");
                mValue = "";
            }else {
                String jid = accountInfo.getOpenfireAccount() + "@" + accountInfo.getXmppServerName();
                mValue = jid;
            }
        }
        switch (mType) {
