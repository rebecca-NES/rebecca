...
const PublicGroupDbStore = require('./dbif');
...
exports.receive = (_globalSnsDB, socket, request, processCallback, callBackResponse) => {
    _log.connectionLog(7, 'do func groupchat.public.api.request(...');
       ...
       !Validation.accessTokenValidationCheck(request.accessToken, true)){
        ...
        case 'getRoomList':
            ...
            _ret; /* = getRoomList(_globalSnsDB, request.accessToken,
                    ...
                    .catch((err)=>{
                        _log.connectionLog(3, '  groupchat.public.api.request getRoomList catch err:'+JSON.stringify(err));
                        //httpレスポンスをここで実行
                        callBackResponse(
                            processCallback,
                            request.accessToken,
...
const getRoomList = (globalSnsDB, accessToken, startId, count) => {
    _log.connectionLog(7, 'do func groupchat.public.api.getRoomList(...');
    return new Promise((resolve, reject) => {
        ...
        if(_sessionData){
            ...
            const publicGroupDb = new PublicGroupDbStore(globalSnsDB, tenantuuId);
            publicGroupDb.getRoomList(
                startId,
                count
            ).then((res)=>{
                resolve({
                ...
                _log.connectionLog(3, '  groupchat.public.api.getRoomList db err:'+ JSON.stringify(err));
                reject({
                    errorCode : 1,
                    content : err
                });
            ...
            _log.connectionLog(3, '  groupchat.public.api.getRoomList not session error');
            reject({
                errorCode : 1,
                content : {
                    result: false,
