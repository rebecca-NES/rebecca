...
export default class ChatScreen extends Component<{}> {
  ...
  render() {
    ...
    var headerTitle = Common.urldecode(this.state.nickname)
    var sendMessage = true
    var tabColor = Const.chat_backcolor
    ...
    if (this.state.single) {
      ...
      sendMessage = false
      tabColor = Const.chat_conversation_color
  ...
  async _fetch_list() {
    ...
    try {
      if (this.state.itemId) {
        ...
        fetchlist = await CubeeAPI.fetchChatList(this.state.jid, this._fetch_lastid)
      }
    }
    catch (e) {
      Common.saveErrorLog(_ScreenName, "チャットメッセージの取得に失敗しました", e)
    }
  ...
  async _onSendMsgBtn() {
    if(this.isTouchable){
      ...
      try {
        await CubeeAPI.sendChatMessage(
          this.state.jid,
          replyid,
