...
export default class GroupChatScreen extends Component<{}> {
  ...
  render() {
    ...
    var tabColor = Const.groupchat_backcolor
    ...
    if (this.state.single) {
      ...
      sendMessage = false
      tabColor = Const.groupchat_conversation_color
  ...
  async _fetch_list() {
    ...
    try {
      if (this.state.itemId) {
        ...
        fetchlist = await CubeeAPI.fetchGroupChatMessgae(this.state.roomid, this._fetch_lastid)
      }
    }
    catch (e) {
      Common.saveErrorLog(_ScreenName, "グループチャットメッセージの取得に失敗しました", e)
    }
  ...
  async _onSendMsgBtn() {
    if(this.isTouchable){
      ...
      try {
        await CubeeAPI.sendGroupChatMessage(
          this.state.roomid,
          replyid,
  ...
  _onPhotoBtn() {
    ...
    ImagePicker.launchImageLibrary(options, (response)  => {
      if (response.didCancel) {
        ...
        this.setState({
          attach_filename: filename,  // 添付ファイル名
          attach_source: response  // 添付ファイルパス
        })
      }
    })
  }
