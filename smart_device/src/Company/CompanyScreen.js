...
export default class CompanyScreen extends Component<{}> {
  ...
  async componentWillReceiveProps(nextProps) {
    ...
    } else if (nextProps.method === "Reply") {
      this._fetch_lastid = 0
      this._fetch_items = []
      this._attaches = []
    ...
    } else if (nextProps.method === "Kaiwa") {
      this._fetch_lastid = 0
      this._fetch_items = []
      this._attaches = []
  ...
  render() {
    ...
    var headerTitle = "フィード"
    var sendMessage = true
    var tabColor = Const.company_backcolor
    ...
    if (this.state.single) {
      ...
      headerTitle = "会話(フィード)"
      sendMessage = false
      tabColor = Const.company_conversation_color
    ...
    if (this._mode !== NORMAL_MODE) {
      leftComponent =
        <Text
          style={{color: Const.company_forecolor, paddingTop:3, paddingBottom:3}}
          onPress={() => this._onPressCloseButton()}
        >
          戻る
        </Text>

      rightComponent = <Text style={{opacity:0}}>戻る</Text>
    }
    this.isTouchable = true
      ...
      <View style={{flex: 1, backgroundColor: Const.basic_backcolor}} >
        <View style={{flex: 1, justifyContent: 'flex-start'}}>
          ...
          <Header
            ...
            centerComponent={{ text: headerTitle, style: {textAlign:'center', color: Const.company_forecolor,width:'65%', fontSize:16, fontWeight:'bold'} }}
              ...
              <View style={{justifyContent: 'center', paddingLeft: 12, paddingRight: 40}}>
                ...
                <View style={{flexDirection: 'row', paddingTop: 5}}>
                  <View style={{flexDirection: 'row', width: '65%'}} >
                    {(() => {
                      if (this._mode !== REPLY_MODE && item.type !== 6) {
                        return (
                    ...
                    {(() => {
                      if (this._mode === NORMAL_MODE && item.type !== 6) {
                        return (
                  ...
                  <View style={{width: '35%', flexDirection: 'row', justifyContent: 'flex-end'}} >
                    {(() => {
                      if (item.type !== Const.cubee_message_system) {
                        return (
                          <TouchableOpacity
  ...
  async _fetch_list() {
    ...
    try {
      if (this.state.itemId) {
        ...
        fetchlist = await CubeeAPI.fetchCompanyList(this._fetch_lastid)
      }
    }
    catch (e) {
      Common.saveErrorLog(_ScreenName, "フィードメッセージの取得に失敗しました", e)
    }
  ...
  async _onSendMsgBtn() {
    if(this.isTouchable){
      ...
      try {
        await CubeeAPI.sendCompanyMessage(
          replyid,
  ...
  _onCameraBtn() {
    ...
    ImagePicker.launchCamera(options, (response)  => {
      if (response.didCancel) {
        //Common.saveErrorLog(_ScreenName, "写真の撮影を中止しました。", "")
  ...
  _onPhotoBtn() {
    ...
    ImagePicker.launchImageLibrary(options, (response)  => {
      if (response.didCancel) {
        //Common.saveErrorLog(_ScreenName, "写真の選択を中止しました。", "")
      } else if (response.error) {
